module.exports = {
  //job concurrency
  concurrency: 3, // Update need slow & waiting for create
  perform: function(job, context, done) {
    ...
    domain.on('error', function(err) { // help dont stuck as active job
      sails.log.error('ShopifyOrderUpdateWorker:domain:error',err);
      done(err);
    });

    // Main process function
    domain.run(async () => {
      sails.log.debug('ShopifyOrderUpdateWorker Run');
